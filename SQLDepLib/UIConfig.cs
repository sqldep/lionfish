...
namespace SQLDepLib
{
    public class UIConfig
    {
        ...
        public static Arguments GetArguments()
        {
            ...
            {
                dbType = Get(SQL_DIALECT, ""),
                server = Get(SERVER_NAME, ""),
                port = Get(SERVER_PORT, ""),
                customSqlSetName = Get(BATCH_NAME, ""),
                sMyKey = Get(SQLDEP_KEY, ""),
                myKey = Guid.Parse(Get(SQLDEP_KEY, "")),
                loginName = Get(LOGIN_NAME, ""),
                loginpassword = Get(LOGIN_PASSWORD, ""),
                ...
                fs_path = Get(FS_PATH, ""),
                fs_mask = Get(FS_MASK, ""),
                fs_default_schema = Get(FS_DEFAULT_SCHEMA, ""),
                fs_default_db = Get(FS_DEFAULT_DB, ""),
                fs_useFs = Get(FS_ENABLED, false),

                ext_InformaticaPath = Get(EXT_INFA_FOLDER, ""),
                ext_SAPPath = Get(EXT_SAP_FOLDER, ""),
                ext_SSISPath = Get(EXT_SSIS_FOLDER, ""),
                ext_useInformatica = Get(EXT_INFA_ENABLE, false),
                ext_useSAP = Get(EXT_SAP_ENABLE, false),
                ext_useSSIS = Get(EXT_SSIS_ENABLE, false),
                ext_SSISMask = Get(EXT_SSIS_MASK, ""),
                ext_SAPMask = Get(EXT_SAP_MASK, ""),
            };
        }
